...
pars_ranger_FUN <- function(params, analysis_set) {
   mtry <- params$mtry[[1]]
   trees <- params$trees[[1]]
   model <- parsnip::rand_forest(mode = "regression", mtry = mtry, trees = trees) %>%
...
mod_FUN_list_pars <- list(glmnet = glm_FUN, ranger = pars_ranger_FUN)
...
# start nested-cv
algorithm_comparison_ten_pars <- map2_dfr(mod_FUN_list_pars, params_list, compare_algs, ncv_dat_10) %>%
   mutate(model = names(mod_FUN_list_pars)) %>%
...
mlflow_set_tag("Implementation", "parsnip")
...
msg <- glue("Using parsnip-kj script: \n After running 10 fold pars,  {log.txt[[1]]}")
pbPost("note", title="parsnip-kj script finished", body=msg)
tic.clearlog()
